{-# LANGUAGE TypeOperators, FlexibleContexts, FlexibleInstances,
  TypeSynonymInstances, MultiParamTypeClasses, ScopedTypeVariables,
  TemplateHaskell, UndecidableInstances, DoAndIfThenElse #-}
--------------------------------------------------------------------------------
-- |
...
import Poets.Contracts.Language.CSL.AST as AST hiding (EQ,GT,LT)
...
inline fDefs e = repeatSubst e
    where subst :: Map Var (Term f)
          subst = Map.map (deepInject3 . functionExp) fDefs
          repeatSubst e =
