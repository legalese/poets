{-# LANGUAGE TypeOperators, TypeSynonymInstances, FlexibleInstances, DoAndIfThenElse #-}
--------------------------------------------------------------------------------
-- |
...
import Poets.Data hiding (RunTimeError, Type)
...
import Poets.Contracts.Language.CSL.Typing.TypeInferer hiding (RunTimeError)
                ...
                case fmap fieldType (getFieldInfo fEnv f) of
                  ...
                  Right tp -> do
                      ...
                      (asubcs,s) <- either (\(MatchError e p) -> throwError $ TypeError e p)
                                           return
                                           (match ((SubTypeConstraint tp tp2 :&: p) : map (fmap deepInject2) subcs') iter')
...
simplify' iter recordEnv pos tp scs fcs ecs ocs = do
  ...
  let subst4 = Map.map deepInject2 subst3 :: TypeSubst
